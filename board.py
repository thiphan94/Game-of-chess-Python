class Board(object):
    ...
    def searchForPawnUpgrade(self, c):
        for x in range(8):
            if c == "White":
                if (self.board[x][0].returnType() == "Pawn") and (
                    self.board[x][0].returnColor() == "White"
                ):
                    return x
            else:
                if (self.board[x][7].returnType() == "Pawn") and (
                    self.board[x][7].returnColor() == "Black"
                ):
                    return x
        return -1
    ...
    def upgradePawn(self, x, y, t):
        ...
        for i in range(
            len(self.destroyedPieces)
        ):  # Remove pieces from list that have been added back to board
            if (
                self.destroyedPieces[i].returnType() == t
                and self.destroyedPieces[i].returnColor()
                == self.board[x][y].returnColor()
            ):
    ...
    def getPoints(self):
        ...
        for x in range(8):
            for y in range(8):
                ...
                if (self.board[x][y].returnType() == "Knight") or (
                    self.board[x][y].returnType() == "Bishop"
                ):
            ...
            if self.pColor == "Black":
                if (self.nType == "blank") and (prevR == newR):
                    ...
                    if (
                        (newC - 2 == prevC)
                        and (prevC == 1)
                        and (self.board[prevR][newC - 1].returnType() == "blank")
                    ):
                        if newR - 1 > -1:  # Prevent IndexError
                            if (self.board[newR - 1][newC].returnType() == "Pawn") and (
                                self.board[newR - 1][newC].returnColor() != "Black"
                            ):
                                self.board[newR - 1][newC].setEnPassantEligible(
                                    True, newR, newC
                                )
                ...
                if (
                    (self.nType != "blank")
                    and (newC - 1 == prevC)
                    and ((newR + 1 == prevR) or (newR - 1 == prevR))
                ):
                    return True
                # En passant
                if (
                    ...
                    if self.board[prevR][prevC].isEnPassantEligible():
                        self.board[self.board[prevR][prevC].getEnPassantAttackX()][
                            self.board[prevR][prevC].getEnPassantAttackY()
            ...
            if self.pColor == "White":
                if (self.nType == "blank") and (prevR == newR):
                    ...
                    if (
                        ...
                        if newR - 1 > -1:  # Prevent IndexError
                            if (self.board[newR - 1][newC].returnType() == "Pawn") and (
                                self.board[newR - 1][newC].returnColor() != "White"
                            ):
                                self.board[newR - 1][newC].setEnPassantEligible(
                                    True, newR, newC
                                )
            ...
            if (newC < prevC) and (newR > prevR):
                while (newC != prevC - 1) and (newR != prevR + 1):
                    ...
                    if self.board[prevR][prevC].returnType() != "blank":
                        return False
                return (newC == prevC - 1) and (
                    newR == prevR + 1
                )  # Final check to ensure diagonal
            # Moving down right
            if (newC > prevC) and (newR > prevR):
                while (newC != prevC + 1) and (newR != prevR + 1):
                    prevR += 1
