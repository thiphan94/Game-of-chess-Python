...
class Chess(object):
    ...
    #             if (
    #                 self.gm == "CPU" and gameOver == False and self.b.checkWin() == ""
    ...
    #         for y in range(8):
    #             if (
    #                 self.b.getBoard()[x][y].returnColor() == color
    #                 and self.b.getBoard()[x][y].returnType() == "King"
    #             ):
    ...
    #         if (self.colorPieceList[i].returnType() != "blank") and (
    #             self.colorPieceList[i].returnType() != "Pawn"
    #         ):
    #             self.onlyPawns = False
    #     # prevent window with only pawns
    #     if not (self.onlyPawns):
    #         # Only create new window if one doesn't exist
    #         if (c == "White" and self.whiteWindowOpened == False) or (
    #             c == "Black" and self.blackWindowOpened == False
    #         ):
    ...
    #         self.destroyedTileList = [
    #             Button(self.upgradeW, width=10, height=5, bg="red")
    #             for x in range(0, len(self.colorPieceList))
    #         ]
    #         print(len(self.destroyedTileList))
    #         self.completeImageList = [
    #             ImageTk.PhotoImage(Image.open("src/images/none_blank.png"))
    #             for x in range(0, len(self.colorPieceList))
    #         ]
    ...
    #                     print("\n" + self.imageLocation)
    #                     self.completeImageList[i] = ImageTk.PhotoImage(
    #                         Image.open(self.imageLocation).resize(
    ...
    #                     )
    #                     self.destroyedTileList[i].configure(
    #                         command=lambda x=x, y=y, i=i, c=c: self.doPawnUpgrade(
    ...
    def loadWindowItems(self):
        ...
        self.tileMatrix = [
            [Button(self.window, width=20, height=10) for x in range(8)]
            for y in range(8)
        ]  # create matrix of chess squares
        self.index = 1
        for r in range(8):
            for c in range(8):
    ...
    def loadMatrix(self):  # updates gui from board matrix
        self.pieceImage = [
            [ImageTk.PhotoImage(Image.open("img/b_pawn.png")) for x in range(8)]
            for y in range(8)
        ]
        for r in range(8):
            for c in range(8):
                ...
                if self.b.getImage(r, c) != "img/w_pawn.png":
                    self.tileMatrix[r][c].configure(
...
cpuA = Button(
    ...
    text="CPU - Algorithm",
    bg="blue",
    command=lambda: startGame("CPU"),
)
cpuN = Button(
    window,
    width=15,
    height=10,
    text="CPU - Neural Network",
    bg="green",
    command=lambda: startGame("NN"),
)
opvp = Button(
    window,
    width=15,
